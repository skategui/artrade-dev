...
export class AllExceptionsFilter implements GqlExceptionFilter {
  constructor(private logger: AppLogger) {}
    ...
    if (['AuthenticationError', 'ForbiddenError', 'UserInputError'].includes(exception.name)) {
      this.logger.error(exception.name, exception.message, gqlHost.getInfo()?.fieldName);
      return exception;
    }
    if (exception.name === 'MongoServerError' && exception.message.includes('E11000')) {
      ...
      this.logger.error(e.name, e.message, gqlHost.getInfo()?.fieldName);
      return e;
    }
    const mongodbObjectIdError =
      'Argument passed in must be a single String of 12 bytes or a string of 24 hex characters';
    if (exception.message === mongodbObjectIdError) {
      this.logger.error('UserInputError', mongodbObjectIdError, gqlHost.getInfo()?.fieldName);
      return exception;
    }
