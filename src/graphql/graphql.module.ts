...
import { GraphQlLamportScalar } from './scalars/lamport.scalar';
...
export const graphqlModule = GraphQLModule.forRootAsync<ApolloDriverConfig>({
  imports: [LoggingModule, EventEmitterModule, jwtModule],
  driver: ApolloDriver,
  useFactory: (
    conf: ConfigService,
    logger: AppLogger,
    ...
    return {
      ...
      resolvers: {
        LamportAmount: GraphQlLamportScalar,
      },
      context: (ctx: { req: Request }) => {
        ...
        try {
          ...
          if (authenticationHeader.data) {
            ...
            const tokenPayload = jwtService.verify<AuthTokenPayload>(authenticationToken);
  ...
  inject: [ConfigService, AppLogger, JwtService, EventEmitter2],
});
